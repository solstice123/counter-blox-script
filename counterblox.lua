...
-- 2. ИНТЕРФЕЙС
local ScreenGui = Instance.new("ScreenGui", CoreGui); ScreenGui.Name = "Semirax_Final_Ultra"
...
CreateSlider("ZOA RADIUS", "Radius", 10, 600, 20)
...
RunService.RenderStepped:Connect(function()
    ...
    if Char and Hum then
        if Flags.BHOP and UserInputService:IsKeyDown(Enum.KeyCode.Space) then
            Hum.Jump = true
            if tick() - LastSpeedUpdate >= 1 then CurrentSpeed = math.clamp(CurrentSpeed + 3, 16, 100); LastSpeedUpdate = tick() end
            ...
            if onScreen and Flags.WH and isEnemy then
                ...
                if head then
                    ...
                    if Flags.Aimbot and IsVisible(head, c) then
                        ...
                        if screenDist < MinDist then MinDist = screenDist; Target = head end
                    end
                end
    ...
    if Target then
        ...
        local smooth = (dist < 20) and 0.25 or 0.45 -- Динамическая плавность для точности вблизи
