...
-- ==========================================
-- 1. СИСТЕМА ТОТАЛЬНОЙ ОЧИСТКИ (ANTI-GHOST)
-- ==========================================
...
-- Удаление старых ESP (Drawing) и Wallhack (Highlights)
if _G.Old_ESP_Data then
    for _, p_esp in pairs(_G.Old_ESP_Data) do
        for _, obj in pairs(p_esp) do 
            if typeof(obj) == "table" or (typeof(obj) == "userdata" and obj.Remove) then 
...
-- ==========================================
-- 3. СОЗДАНИЕ ИНТЕРФЕЙСА (VFX EDITION)
-- ==========================================
local ScreenGui = Instance.new("ScreenGui", CoreGui); ScreenGui.Name = "Semirax_Ultimate_V8"
...
Gradient.Color = ColorSequence.new({ColorSequenceKeypoint.new(0, Color3.fromRGB(5, 5, 20)), ColorSequenceKeypoint.new(1, Color3.fromRGB(30, 50, 110))})
...
FuncBtn.MouseButton1Click:Connect(function() 
    ...
    TweenService:Create(FuncBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(40, 60, 140)}):Play()
...
BindBtn.MouseButton1Click:Connect(function() 
    ...
    TweenService:Create(BindBtn, TweenInfo.new(0.3), {BackgroundColor3 = Color3.fromRGB(40, 60, 140)}):Play()
...
local function CreateElement(name, flag)
    ...
    btn.MouseButton1Click:Connect(function()
        ...
        local col = Flags[flag] and Color3.fromRGB(0, 120, 255) or Color3.fromRGB(30, 30, 45)
...
UserInputService.InputBegan:Connect(function(i, g)
    if not g and Binds[i.KeyCode] then
        local d = Binds[i.KeyCode]; Flags[d.Flag] = not Flags[d.Flag]
...
local function AddESP(p)
    ...
    d.Tag.Size = 13; d.Tag.Color = Color3.new(1,1,1); d.Tag.Outline = true; d.Tag.Center = true
...
Players.PlayerRemoving:Connect(function(p) if ESP_Data[p] then for _, v in pairs(ESP_Data[p]) do pcall(function() v:Remove() v:Destroy() end) end ESP_Data[p] = nil end end)
...
RunService.RenderStepped:Connect(function()
    ...
    if Flags.BHOP and UserInputService:IsKeyDown(Enum.KeyCode.Space) and LocalPlayer.Character then
    ...
    for p, d in pairs(ESP_Data) do
        ...
        if c and h and r and h.Health > 0 then
            ...
            if onScreen and Flags.WH and (not Flags.TeamCheck or isEnemy) then
                ...
                if head then
                    local tP = Camera:WorldToViewportPoint(head.Position + Vector3.new(0, 0.7, 0)); local bP = Camera:WorldToViewportPoint(r.Position - Vector3.new(0, 3, 0))
