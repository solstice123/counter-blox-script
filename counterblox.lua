...
-- ФУНКЦИЯ ПРОВЕРКИ ВИДИМОСТИ (Чтобы не палить триггер через стены)
...
-- 2. ИНТЕРФЕЙС
local ScreenGui = Instance.new("ScreenGui", CoreGui); ScreenGui.Name = "Semirax_Trigger_V12"
...
local function AddESP(p)
    ...
    local d = ESP_Data[p]; d.Box.Thickness = 1.5; d.Box.Color = Color3.new(1,1,1); d.Tag.Size = 14; d.Tag.Color = Color3.new(1,1,1); d.Tag.Outline = true; d.Tag.Center = true; d.BarBack.Filled, d.BarBack.Color, d.BarBack.Transparency = true, Color3.new(0,0,0), 0.5; d.Bar.Filled = true
end
...
RunService.RenderStepped:Connect(function()
    ...
    -- TRIGGERBOT (ЗАЖИМ МЫШКИ)
    if Flags.TriggerBot then
        local mouse = LocalPlayer:GetMouse()
        local target = mouse.Target
        if target and target.Parent and target.Parent:FindFirstChild("Humanoid") then
            local tChar = target.Parent; local tPlayer = Players:GetPlayerFromCharacter(tChar)
            if tPlayer and (not Flags.TeamCheck or tPlayer.Team ~= LocalPlayer.Team) then
                if IsVisible(target, tChar) then
                    if not IsPressing then IsPressing = true; mouse1press() end
                else if IsPressing then IsPressing = false; mouse1release() end end
            else if IsPressing then IsPressing = false; mouse1release() end end
        else if IsPressing then IsPressing = false; mouse1release() end end
    end
